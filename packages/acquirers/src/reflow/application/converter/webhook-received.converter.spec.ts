...
import { ReflowWebhookReceivedConverter } from './webhook-received.converter';
import { PaymentMethodEnum } from '../dto/payment-method-enum';

describe('ReflowWebhookReceivedConverter', () => {
  let converter: ReflowWebhookReceivedConverter;

  beforeEach(() => {
    converter = new ReflowWebhookReceivedConverter();
  });
  ...
  const scenarios = [
    {
      ...
      testFn: async () => {
        ...
        expect(transactions[0].getAcquirer()).toBe(AcquirerEnum.REFLOW);
